...
### 1.1 Installing
First of all, `voice` and `birdclef2023` packages are installed.
[`voice`](https://github.com/filipezabala/voice) contains functions to deal with audio, and [`birdclef2023`](https://github.com/filipezabala/birdclef2023) contains summarized data from Kaggle's BirdCLEF2023 competition audios.
...
## 2. Data
In this section is presented how to download, extract and summarize the bird's audio features, and be ommited in a first reading.
...
# distances
D <- matrix(NA, 
            nrow = nrow(birdclef2023::Ets), 
            ncol = nrow(birdclef2023::Esc))
rownames(D) <- birdclef2023::Ets$row_id
colnames(D) <- birdclef2023::Esc$species_code
```

The following code is completly not optimized, designed to be intuitive and run below the 2 hours limit defined in the Kaggle BirdCLEF2023 competition rules. Takes around 30 minutes to run on a local machine.
...
P <- matrix(NA, nrow = nrow(Ets), ncol = nrow(Esc))
rownames(P) <- Ets$row_id
colnames(P) <- Esc$species_code

# calculating probabilities
for(i in 1:nrow(P)){
  nm <- gaussian(scale(D[i,]), mu = 0, sigma = 0.001)
  P[i,] <- nm/sum(nm)
}
```
...
submission <- tibble(block = block, row_id = rownames(P), as_tibble(P))
